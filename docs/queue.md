...
| # | Ticket | Note |
|---|---|---|
| N1 | **TET-96** — typed legs in the mint-time verifier: Jev as gate, classify, literal judge and refuter | In progress, as three PRs. Slice 1 (plumbing and the typed refuter on `fact`: routing, the per-verb table, the `verify.model` refusal, credentials, per-provider budget, pricing, version flag) **merged** (PR #15, 2026-09-21); its review added `RefuterLeg::Itself` and a `reason` on `refuter_not_run`. Slice 2 is the gates, `verify.typed_model`, `Status::Gated` and `gate_incomplete`, plus the tests for invariants 2, 3, 5 and 10, which slice 1 deferred; slice 3 is Jev classify and literals on `claim`. Nothing blocks it: TET-85 covers `verify.enabled = false`, not the enabled-path `unauthorized` arms this adds. Ships with both new defaults off. |
