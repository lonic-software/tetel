...
| A16a | **TET-99** — a refused `verify.typed_model` value turns the typed legs off without a word in the reply | Same class as A16: a setting whose effect the operator cannot see. `verify.model`'s refusal already reaches `detail`; mirror it. Found in review of PR #23, predates it. Small. |
| A17 | **TET-84** — `verify` returns `unavailable` on 31% of mints | TET-98's timings are on the ticket: the default budget would cut off 11–30% of the draws that ran the check, the ones most likely to carry findings. A third of the mint-time verifier's work is lost after a full timeout and four attempts, so it also costs the wall-clock it wasted. |
