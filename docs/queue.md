...
| A5 | **TET-95** — `run` returns its whole capture | ~125k characters in one session, mostly a test runner's passing lines. The capture must stay whole as evidence; only the reply needs bounding. **Done** (PR #43). Measured first, and the premise did not hold: over 5,716 unique `run` replies in local agent transcripts, test and build commands were 22 replies and 0.1% of the bytes. The large replies were reads (grep, sed, awk), and a head/tail bound at 8 KiB hid a span a later `fact` note quoted in 203 of the 286 replies between 8 and 32 KiB (80 at 16 KiB). So nothing is cut below the budget and there is no opt-out. What shipped: `run` fits its own over-budget reply (whole lines by JSON-escaped cost; the next line cut inside itself when a line boundary would give up more than half the room), with `omitted` ahead of `output` saying what was shown and that the capture keeps every line. Its two refusals cut the echoed command line to `ENTRY_CAP`. The backstop's real-client test now drives its cut through `render`. Left open: the tool description does not mention `omitted` (an edit risks TET-91's Fable threshold), and the label does not record a partial view (labels are token-searched for coverage). |
| A6 | **TET-72** — `check` outgrows its own consumers | 22–31k characters on a 16-claim memo, 75k on a 30-claim one. `check` is called repeatedly during the loop, so its size multiplies. Not a licence to re-open TET-51's rejected trim — the residue stays printed item by item. **Done** (PR #44). Paging, not a summary or JSON: JSON would halve the room, since the reply size counts structured content too. Over MCP, an over-budget report pages by row. Each page leads with the rows shown, the rows left out counted by kind, and `from: N` to continue. Every page repeats the partition headers and the build line. A group's preamble prints on any page that shows one of its entries. A `from` that names no row is refused as is_error. The CLI still prints the whole report; its failing header now counts rows by kind (`3 failing (grammar 2, subset 1)`), read off the rows rather than summed by hand. Left open: a single fact's row that outgrows a page is cut and points at the CLI (TET-105; no design memo produces one today). |
| A7 | **TET-73** — failing partition reprints each prior verdict | **Done** (closed 2026-09-26). The fix, `bf8079f` (PR #9), was on main but never transitioned. Verified at tip: `out-of-proof` and `superseded evidence` print one row per claim, each with its stale count, the remedy and a `grep` pointer to the ledger. No verdict note is reprinted. The pinning test `several_stale_records_on_one_claim_collapse_to_one_row_each` goes red at its row-count assertion when the push is repeated per stale record. On all 12 memos in `docs/design`, superseded rows equal distinct claims (`tet47`: 338 records, 21 rows). Out-of-proof is 0 on every real memo, so that partition rests on the fixture. The ticket's `query --superseded` was not built; the grep pointer stands in for it. |
| A8 | **TET-66** — a wrong-block `prose --revise` is indistinguishable from a right one | **Done** (PR #45). Measured first. Across the 13 memos in `docs/design`, the `why` of a later revision records five prose revisions that hit the wrong block (two of them headings), one that dropped a paragraph, and one claim revision with the wrong cites. That is out of about 660 prose and 400 claim revisions. So the fix covers every act on an existing id, not just `prose --revise`. Prose revise and ack, fact revise, claim revise and withdraw, and target and transplant withdraw each reply with `was`: the object as it stood before the act. `was` gives the first non-blank line (160 escaped bytes), the ids it cites and, for prose, the kind, the level and the paragraph count before and after. It also says when a claim was already withdrawn. The cites are echoed even when unchanged, because four of the five wrong-block revisions passed no new cites. The CLI prints the same content as a second line, under the unchanged first. It fits inside `ENTRY_CAP` beside a maximal `verify`. `fact` places it before fitting its lists. Rejected: an opt-in compare-and-swap `expect`, echoing the whole old text, and `query prose --ids` (the deferral in `tet93-bounded-reply` C13 ends here, unbuilt). No tool description changed (TET-91). Left for B6 (TET-64): a warning, as a sibling key beside `was`. |
