...
| A17 | **TET-84** — `verify` returns `unavailable` on 31% of mints | **Two of three parts done** (2026-09-23). The 19 `unavailable` mints were budget expiry reported under the wrong status. The provider sends its headers at once and its body when the model is done, so the budget ran out during the body read, and that path reported `unavailable` / "reply body could not be read". It is now `timeout` (PR #28). It was not out-of-credit: that is a 402, which already had its own detail. The "four attempts" were the verification's calls across its legs, not retries. The default budget is now 100s per OpenRouter leg, up from 60s (PR #29), because on `gpt-6-luna` 60s still cut off 12 of 113 answered gated `fact` draws. **Part three designed** (2026-09-23, `docs/design/tet84-loud-status.md`), not yet built. A failed verification's reply carries its `detail`, with reply text kept in the log. `guidance` says the mint was not checked. A new `unverified` key names every mint whose latest verification failed, on every reply, until it is re-verified. Re-running a verification without changing the text is deferred to TET-100. |
